...
def analizar_comida(image: Image.Image):
    ...
    try:
        ...
        # SI FALLA, EJECUTAMOS EL DIAGNÓSTICO DE MODELOS
        lista_modelos = []
        try:
            ...
            lista_modelos = ["No se pudo conectar a Google para listar modelos"]

        # Lanzamos un error especial mostrando qué modelos SÍ están disponibles
        mensaje_error = f"""
        ❌ Error usando '{nombre_modelo}'.
        
        📋 MODELOS DISPONIBLES EN TU SERVIDOR:
        {', '.join(lista_modelos)}
        
        🔍 Detalle técnico: {e_principal}
        """
        ...
        if st.button("Analizar comida"):
            with st.spinner("Analizando con IA..."):
                try:
                    ...
                    # AQUÍ SE MOSTRARÁ LA LISTA DE MODELOS REALES
                    st.error(e)
